...
def scale_coords(img1_shape, coords, img0_shape, ratio_pad=None):
    # Rescale coords (xyxy) from img1_shape to img0_shape 还原box坐标
...
def postprocess(output, device='cpu', conf_thres=0.25, iou_thres=0.45, max_det=1000, ):
    output = torch.tensor(output.reshape(-1, 25200, 85), device=device)
    output = non_max_suppression(output, conf_thres, iou_thres, None, False, max_det=max_det)
    return output


def draw_images(pred, image):
