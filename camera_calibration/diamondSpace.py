...
def points_to_paras(points):
    paras = []
    for point in points:
        x1, y1, x2, y2 = point.ravel()
        a = (y2 - y1) / (x2 - x1)
        c = y1 - a * x1
        paras.append([-a, 1, -c])
    return paras
...
class DiamondSpace:
    ...
    def relevant_intersections(self, points):
        ...
        return points, np.logical_and(np.logical_and(X, Y),
                                      np.logical_and(np.logical_and(X, Y), regular_points[:, :, 0]))
    ...
    def filter_lines_from_peak(self, peak, lines):
        ...
        self.clear()
        self.insert(np.array(new_lines))
        return index
    ...
    def attach_spaces(self):
        ...
        accumulator[0: self.size, self.size - 1:] = np.maximum(np.flipud(self.A[2]),
                                                               accumulator[0: self.size, self.size - 1:])
        accumulator[self.size - 1:, self.size - 1:] = np.maximum(self.A[1], accumulator[self.size - 1:, self.size - 1:])
        accumulator[self.size - 1:, 0: self.size] = np.maximum(np.fliplr(self.A[0]),
                                                               accumulator[self.size - 1:, 0: self.size])
